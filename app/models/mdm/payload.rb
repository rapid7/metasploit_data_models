class Mdm::Payload < ActiveRecord::Base
  ...
  #
  def load_uri(uri)
    filter_invalid(parse_uri(uri))
  end

  def load_new
    parse_raw(generate_raw)
  end
  ...
  #
  def to_s
    arch_id   = find_architecture_id(self.arch).to_s
    plat_id   = find_platform_id(self.platform).to_s
    [
        self.puid_hex,
  ...
  def arch=(arch_str)
    ...
    arch_id   = find_architecture_id(arch_str)
    if arch_id == 0
      raise ArgumentError, "Invalid architecture: '#{arch_str}'"
    end

    arch_name = find_architecture_name(arch_id)
    @arch = arch_name
  end
  ...
  def platform=(plat_str)
    ...
    plat_id = find_platform_id(plat_str)
    if plat_id == 0
      raise ArgumentError, "Invalid platform: '#{plat_str}'"
    end

    plat_name = find_platform_name(plat_id)
    @platform = plat_name
  end
  ...
  after_initialize do
    ...
    # if opts[:seed]
    #   self.puid = seed_to_puid(opts[:seed])
    # end
  ...
  def find_platform_name(num)
    Platforms[num]
  end

  def find_architecture_name(num)
    Architectures[num]
  end
